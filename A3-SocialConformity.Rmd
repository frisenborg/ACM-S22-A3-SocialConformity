...
# convert to 0 / 1
df$FirstRating_oh <- ifelse(df$FirstRating <= 4, 1, 0)
df$SecondRating_oh <- ifelse(df$SecondRating <= 4, 1, 0)
df$OtherRating_oh <- ifelse(df$OtherRating <= 4, 1, 0)
...
file <- file.path(paste(stan_path, "W8_simpleBayes.stan", sep=""))
stan_model <- cmdstan_model(file, cpp_options = list(stan__threads = TRUE))
