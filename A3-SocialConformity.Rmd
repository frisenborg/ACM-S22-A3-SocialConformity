...
file <- file.path("stan_models/W8_simpleBayes.stan")
...
file_c <- file.path("stan_models/Continuous_Simple.stan")
stan_model <- cmdstan_model(file_c, cpp_options = list(stan__threads = TRUE))

data <- list(
  N       = length(df_subset$FirstRating_c),
  y       = df_subset$SecondRating_c,
  Source1 = df_subset$FirstRating_c,
  Source2 = df_subset$OtherRating_c)
...
prediction_df_long <- pivot_longer(prediction_df, cols=c("true_rating", "prediction"))
...
file_c <- file.path("stan_models/bayes_with_w8s.stan")
stan_model <- cmdstan_model(file_c, cpp_options = list(stan__threads = TRUE))
